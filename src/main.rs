...
async fn handle_chat_completions(
    ...
    if is_streaming {
        ...
        let result = handle_non_streaming_generation(
            headers,
            Json(payload),
            detectors,
            gateway_config,
...
async fn orchestrator_streaming_request(
    ...
    let response = req.send().await.map_err(|e| {
        tracing::error!("Failed to send streaming request or connect to orchestrator: {:?}", e);
        anyhow::anyhow!("Failed to send streaming request or connect to orchestrator: {:?}", e)
    })?;

    let status = response.status();
    if !status.is_success() {
        let error_text = response.text().await.unwrap_or_else(|_| "Unknown error".to_string());
        tracing::error!("Orchestrator returned error status {}: {}", status, error_text);
        return Err(anyhow::anyhow!(
            "Orchestrator returned error status {}: {}",
            status,
            error_text
